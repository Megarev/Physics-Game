...
public:
    ...
    void ApplyForce(const olc::vf2d& force, float dt) { if (mass > 0.0f) velocity += force * dt; }
    ...
    const float& GetPrevAngle() const { return prev_angle; }
    void SetPrevAngle(float a) { prev_angle = a; }
...
void RigidBody::Logic(float dt, float alpha, bool is_debug) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        // Translation
        vertices[i] += alpha * prev_pos + (1.0f - alpha) * position;
    }
...
void Manifold::SetState() {
    ...
    if (a->n_iter > a->n_iter_update) {
        ...
        if (std::fabsf(a->GetAngle() - a->GetPrevAngle()) <= 0.01f) {
            is_rotational_move = false;
            a->angular_velocity = 0.0f;
        }
    ...
    if (b->n_iter > b->n_iter_update) {
        ...
        bool is_rotational_move = true;
        if (std::fabsf(b->GetAngle() - b->GetPrevAngle()) <= 0.01f) {
            is_rotational_move = false;
...
void Manifold::ApplyForces(float dt) {
    ...
    for (const olc::vf2d& p : points) {
        ...
        olc::vf2d t = (rv - rv_normal * normal); // Tangent from triangle law with rv and rv_normal vector
        if (t.mag2() > EPSILON) t = t.norm();
...
void Scene::Update(float dt, bool is_debug) {
    ...
    if (accumulator > inv_FPS) {
        ...
        for (int i = shapes.size() - 1; i >= 0; i--) {
            if (shapes[i].mass > 0.0f && is_gravity && shapes[i].is_move) shapes[i].AddAcceleration({ 0.0f, g });
